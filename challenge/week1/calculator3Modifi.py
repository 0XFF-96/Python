...
class Args(object):
	...
	def config_path(self):
		...
		return self._value_agter_option('-c')
...
class IncomeTaxCalculator(object):
	...
	cls.calc_social_insurance_money(income)
	
		real_income = income - social_insurance_money
		taxabel_part = real_income - INCOME_TAX_START_POINT
		...
		if taxable_part <= 0:
			...
			for item in INCOME_TAX_QUICK_LOOKUP_TABLE:
				...
				if taxable_part > item.start_point:
					...
					return '{:.2f}'.format(tax), '{:.2f}'.format(real_income_tax)
	...
	def calc_for_all_userdata(self):
		...
		result = []
		for empoyee_id, income in self.userdata:
			data = [employee_id, income]
			social_insurance_money = 
'{:2f}'.format(self.calc_socail_insurance_money(income))

			tax, remain = self.calc_income_tax_and_remain(income)
			data += [social_insurance_money, tax, remain]
			result.append(data)
	def export(self, default='csv'):
		result = self.calc_for_all_uerdata()
		with open(args.exprot_path, 'w', newline='') as f:
			writer = csv.write(f)
